...
```bash
php artisan vendor:publish --tag=ichava::icon-sets-emoji-config
```

That writes `config/icon-sets-emoji.php`, read back as `ichava.icon-sets-emoji`. The defaults are merged without publishing,
